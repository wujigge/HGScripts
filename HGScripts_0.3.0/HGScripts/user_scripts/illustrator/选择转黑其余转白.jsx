...
(function () {
    ...
    for (var i = 0; i < selectedItems.length; i++) {
        applyColor(selectedItems[i], black);
    }
    ...
    for (var k = 0; k < doc.pageItems.length; k++) {
        applyWhiteToUnselectedInArtboard(doc.pageItems[k], artRect, selectedMap, white);
    }
})();

function tryRunCppPlugin() {
    return tryRunHGScriptsCppCommand("HGSelectionBlackOthersWhite");
}
...
function applyWhiteToUnselectedInArtboard(item, artRect, selectedMap, white) {
    if (!isProcessable(item)) return;

    if (item.typename === "GroupItem") {
        for (var i = 0; i < item.pageItems.length; i++) {
            applyWhiteToUnselectedInArtboard(item.pageItems[i], artRect, selectedMap, white);
        }
        return;
    }
    ...
    if (item.typename === "CompoundPathItem") {
        for (var j = 0; j < item.pathItems.length; j++) {
            applyWhiteToUnselectedInArtboard(item.pathItems[j], artRect, selectedMap, white);
        }
        return;
    }
