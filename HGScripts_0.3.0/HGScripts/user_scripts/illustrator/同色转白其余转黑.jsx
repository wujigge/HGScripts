...
(function () {
    ...
    applyMaskColorToDocument(doc, artRect, sample.key, white, black);
    flushSampleDebugLog();
})();

function tryRunCppPlugin() {
    return tryRunHGScriptsCppCommand("HGSameWB");
}
...
function getSingleColorSample(item) {
    resetSampleDebugLog("\u540c\u8272\u8f6c\u767d\u5176\u4f59\u8f6c\u9ed1.jsx");
...
function applyMaskColorToDocument(doc, artRect, sampleKey, white, black) {
    for (var i = 0; i < doc.pageItems.length; i++) {
        var item = doc.pageItems[i];
        if (isTopLevelPageItem(item)) {
            applyMaskColor(item, artRect, sampleKey, white, black);
        }
    }
}
...
function applyMaskColor(item, artRect, sampleKey, white, black) {
    if (!isProcessable(item)) return;

    if (item.typename === "GroupItem") {
        for (var i = 0; i < item.pageItems.length; i++) {
            applyMaskColor(item.pageItems[i], artRect, sampleKey, white, black);
        }
        return;
    }
    ...
    if (item.typename === "CompoundPathItem") {
        for (var j = 0; j < item.pathItems.length; j++) {
            applyMaskColor(item.pathItems[j], artRect, sampleKey, white, black);
        }
        return;
    }
    ...
    if (item.typename === "TextFrame") {
        applyMaskColorToText(item, sampleKey, white, black);
        return;
    }

    applyMaskColorToDirectObject(item, sampleKey, white, black);
}

function applyMaskColorToDirectObject(item, sampleKey, white, black) {
    var changed = false;
    ...
    try {
        if (item.filled) {
            ...
            addApplyDebugLine("APPLY_FILL type=" + item.typename + " name=" + safeDebugName(item.name) + " from=" + fillKey + " matched=" + fillMatched + " to=" + (fillMatched ? "white" : "black"));
            item.fillColor = fillMatched ? white : black;
            changed = true;
        }
    } catch (e1) {}
    ...
    try {
        if (item.stroked) {
            ...
            addApplyDebugLine("APPLY_STROKE type=" + item.typename + " name=" + safeDebugName(item.name) + " from=" + strokeKey + " matched=" + strokeMatched + " to=" + (strokeMatched ? "white" : "black"));
            item.strokeColor = strokeMatched ? white : black;
            changed = true;
        }
    } catch (e2) {}
...
function applyMaskColorToText(textFrame, sampleKey, white, black) {
    try {
        var attrs = textFrame.textRange.characterAttributes;
        if (attrs.fillColor && attrs.fillColor.typename !== "NoColor") {
            var fillKey = colorToKey(attrs.fillColor);
            var fillMatched = fillKey === sampleKey;
            addApplyDebugLine("APPLY_TEXT_FILL name=" + safeDebugName(textFrame.name) + " from=" + fillKey + " matched=" + fillMatched + " to=" + (fillMatched ? "white" : "black"));
            attrs.fillColor = fillMatched ? white : black;
        }
        if (attrs.strokeColor && attrs.strokeColor.typename !== "NoColor") {
            var strokeKey = colorToKey(attrs.strokeColor);
            var strokeMatched = strokeKey === sampleKey;
            addApplyDebugLine("APPLY_TEXT_STROKE name=" + safeDebugName(textFrame.name) + " from=" + strokeKey + " matched=" + strokeMatched + " to=" + (strokeMatched ? "white" : "black"));
            attrs.strokeColor = strokeMatched ? white : black;
        }
    } catch (e) {}
}
