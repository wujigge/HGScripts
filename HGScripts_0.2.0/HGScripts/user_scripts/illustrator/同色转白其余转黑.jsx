...
(function () {
    ...
    for (var i = 0; i < doc.pageItems.length; i++) {
        applyMaskColor(doc.pageItems[i], artRect, sample.key, white, black);
    }
})();
...
function applyMaskColor(item, artRect, sampleKey, white, black) {
    ...
    try {
        if (item.filled) {
            item.fillColor = colorToKey(item.fillColor) === sampleKey ? white : black;
        }
    } catch (e1) {}

    try {
        if (item.stroked) {
            item.strokeColor = colorToKey(item.strokeColor) === sampleKey ? white : black;
        }
    } catch (e2) {}
}
...
function applyMaskColorToText(textFrame, sampleKey, white, black) {
    try {
        ...
        if (attrs.fillColor && attrs.fillColor.typename !== "NoColor") {
            attrs.fillColor = colorToKey(attrs.fillColor) === sampleKey ? white : black;
        }
        if (attrs.strokeColor && attrs.strokeColor.typename !== "NoColor") {
            attrs.strokeColor = colorToKey(attrs.strokeColor) === sampleKey ? white : black;
        }
    } catch (e) {}
}
