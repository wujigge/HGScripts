(function () {
  ...
  function bindEvents() {
    ...
    el.scanDepthInput.addEventListener("change", saveSettingsFromForm);
  ...
  function sortList(list, query) {
    if (query) {
      list.sort(function (a, b) {
        return getSearchScore(b, query) - getSearchScore(a, query) ||
          getRunStat(b.key).lastRun - getRunStat(a.key).lastRun ||
          compareByMtimeDesc(a, b);
      });
    } else if (state.activeSceneId === "recent") {
      list.sort(function (a, b) {
        return getRunStat(b.key).lastRun - getRunStat(a.key).lastRun || compareByMtimeDesc(a, b);
      });
    } else if (state.activeSceneId === "favorites") {
      list.sort(function (a, b) {
        return getFavoriteTime(b) - getFavoriteTime(a) ||
          getRunStat(b.key).lastRun - getRunStat(a.key).lastRun ||
          compareByMtimeDesc(a, b);
      });
    } else if (el.sortSelect.value === "nameAsc") {
