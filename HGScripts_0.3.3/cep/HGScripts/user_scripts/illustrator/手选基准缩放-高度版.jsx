...
// hgscripts-cpp-command: HGBasisScaleToHeight

(function () {
    var SCRIPT_NAME = "手选基准缩放-高度版";
    var DEFAULT_TARGET_HEIGHT = "1024";
    var EPSILON = 0.01;
    ...
    var targetInput = prompt("输入基准对象的目标高度：", getTargetInput(), SCRIPT_NAME);
    if (targetInput === null) {
        return;
    }

    var targetHeight = parseLengthToPoints(targetInput);
    if (!(targetHeight > EPSILON)) {
        return;
    }

    if (writeBasisScaleTargetHeight(targetHeight) && tryRunCppPlugin()) {
        return;
    }

    var currentHeight = getItemHeight(basisItem);
    ...
    var scalePercent = targetHeight / currentHeight * 100;
    ...
    function getTargetInput() {
        try {
            if (typeof HGBASIS_SCALE_TARGET_HEIGHT !== "undefined") {
                return String(HGBASIS_SCALE_TARGET_HEIGHT);
            }
        } catch (e) {}
    ...
    function tryRunCppPlugin() {
        ...
        try {
            app.executeMenuCommand("HGBasisScaleToHeight");
            return true;
        } catch (e) {
            return false;
        }
    ...
    function writeBasisScaleTargetHeight(targetHeight) {
        try {
            var file = new File(Folder.temp + "/HGBasisScaleTargetHeight.txt");
            file.encoding = "UTF-8";
            if (!file.open("w")) {
                return false;
            }
            file.write(String(targetHeight));
            file.close();
            return true;
        } catch (e) {
            return false;
