(function () {
  ...
  var DEFAULT_FOLDER = path.join(EXTENSION_ROOT, "user_scripts");
  var EXTERNAL_DEFAULT_FOLDER = path.join(CEP_ROOT, "user_scripts");
  var EXTERNAL_DATA_FOLDER = path.join(CEP_ROOT, "data");
  var LEGACY_FOLDER_KEY = "hgscripts.folder";
  var SOURCES_KEY = "hgscripts.sources";
  var ACTIVE_SOURCE_KEY = "hgscripts.activeSource";
  var STATE_KEY = "hgscripts.lastState";
  var FAVORITES_KEY = "hgscripts.favorites";
  var RUN_STATS_KEY = "hgscripts.runStats";
  var RECENT_SOURCES_KEY = "hgscripts.recentSources";
  var CONSOLE_HEIGHT_KEY = "hgscripts.consoleHeight";
  var PANEL_SIZES_KEY = "hgscripts.panelSizes";
  ...
  var state = {
    ...
    panelMode: "narrow",
    panelSizes: loadJson(PANEL_SIZES_KEY, null) || {
      narrow: { width: NARROW_PANEL_WIDTH, height: NARROW_PANEL_HEIGHT },
  ...
  function renderAppVersion() {
    ...
    document.title = "\u6D77\u54E5\u7684Adobe\u811A\u672C\u7BA1\u7406\u5668 dev v" + version;
    if (el.appVersion) {
  ...
  function normalizePanelSize(size, fallback) {
    ...
    if (!next.width || next.width < 320) {
      next.width = fallback.width;
    }
    if (!next.height || next.height < 300) {
      next.height = fallback.height;
    }
    next.width = Math.max(320, Math.min(1400, next.width));
    next.height = Math.max(300, Math.min(1800, next.height));
    return next;
  }
  ...
  function scanSources() {
    ...
    if (!existingCount) {
      el.folderHint.textContent = "没有可读取的脚本目录";
      setToast("目录不存在，请重新选择", "error");
    } else {
      el.folderHint.textContent = state.sources.length + " 个目录源，" + files.length + " 个脚本" +
  ...
  function showDetail(item) {
    ...
    var runText = stat.lastRun ? " / 上次运行 " + formatTime(new Date(stat.lastRun)) + " / 运行 " + stat.count + " 次" : "";
  ...
  function runScriptForItem(item, scriptPath, displayPath) {
    ...
    var script = "HGScripts.runScript(" + jsxString(scriptPath) + ")";
  ...
  function updateSavedScriptMetadata(fullPath) {
    var item = findByPath(fullPath);
    if (!item) {
      return;
    }
  ...
  function normalizeFsPath(value) {
    var normalized = normalizeInputPath(value).replace(/\//g, "\\");
